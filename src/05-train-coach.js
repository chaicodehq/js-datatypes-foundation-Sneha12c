...
export function findPassenger(passengers, name) {
  ...
  return passengers.find((num)=>{
    return (num.name.toLowerCase() === name.toLowerCase());
  });
}
...
export function getPassengerIndex(passengers, name) {
  ...
  return passengers.findIndex((num)=>{
     return (num.name.toLowerCase() === name.toLowerCase());
  })
}
...
export function isAnyWaitlisted(passengers) {
  ...
  return passengers.some((obj)=>{
    return obj.status === "waitlisted";
  })
}
